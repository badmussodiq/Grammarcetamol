...
>
> **Task 30 complete (2026-08-06).** The `curl`-based auth-boundary sweep ran clean against all three Phase 3 services through the real gateway (logged in as a real SUPER_ADMIN and a real STUDENT, not mocked): 24 checks — every write/admin-gated endpoint on `enrollment-service`, `payment-service`, and `review-service` correctly 401s with no token and 403s for the wrong role (`GET /api/enrollments/at-risk`, `GET /api/enrollments/user/{id}`, `GET /api/payments`, `GET /api/payments/revenue/summary`, `POST /api/payments/{id}/refund`, `GET /api/reviews`, `GET /api/reviews/{id}`, `PATCH /api/reviews/{id}/moderate`), with an admin-token sanity check on each confirming the endpoint still works for the right caller. One apparent mismatch on first pass — `refund` returning `400` instead of `403` for a STUDENT token — turned out to be the test's own fault: an empty `{}` body failed `RefundPaymentDto`'s `class-validator` checks before the controller's `SUPER_ADMIN` role check ever ran; re-sent with a validation-passing body (`{amount, reason}`), it correctly returned `403 "Only super admins can issue refunds"`. The `PLAN.md`/`implementation-phases.md` per-task status-note pass for Tasks 19–29 was completed the same day. Phase 3 is now fully closed out.
