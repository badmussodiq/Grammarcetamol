...
>
> **Still open:** the final `curl`-based auth-boundary sweep (401/403 checks across `enrollment-service`/`payment-service`/`review-service`) hasn't been done yet. The `PLAN.md`/`implementation-phases.md` per-task status-note pass for Tasks 19–29 is now done (2026-08-06) — every status block above and in `implementation-phases.md`'s Phase 3 section reflects the real state as of this update.
