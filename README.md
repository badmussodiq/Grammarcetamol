...
**Phase 4 — Live Classes & Notifications** — **in progress**. Tasks 38 (`payment-service` subscription billing — recurring Paystack charges, group and negotiated-price private billing) and 39 (`backend/live-class-service` — persistent Classes with independent live Sessions, moderated chat, materials, backend-enforced join-room authorization, instructor scheduling conflict detection) are done and live-verified. Also already sitting in the codebase from earlier Notification Service work: a working in-app notification inbox (backend list/read/unread-count/delete endpoints, plus a student bell icon and `/notifications` page) — nothing populates it yet, that's Task 40. See [`PHASE4.md`](./PHASE4.md) for the living task-by-task tracker.
