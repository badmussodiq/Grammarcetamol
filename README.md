...
below) automatically deploys to a cloud server over SSH — same Docker, no Kubernetes.
**No git on the server**: it only ever runs pre-built images pulled from the registry, so
each deploy just copies `docker-compose.yml` + `docker/` fresh via scp, then runs
`docker/scripts/deploy.sh <commit-sha>` remotely, which pulls that exact commit's images
and restarts. Rolling back is re-running that same script by hand with an older commit
SHA — every commit's images stay in the registry under their own tag.
...
3. Create a real `/opt/grammarcetamol/.env` by hand (copy `.env.example`, fill in real
   secrets). This file is **never** touched by deploys — only `docker-compose.yml` and
   `docker/` get overwritten each time. Set, specifically:
   - `IMAGE_PREFIX=ghcr.io/<owner>/grammarcetamol-` (or your Docker Hub prefix)
   - `IMAGE_PULL_POLICY=always`
   - `COMPOSE_PROFILES=proxy`, `DOMAIN_APP`/`DOMAIN_ADMIN`/`DOMAIN_API`, `CERTBOT_EMAIL` (see nginx/TLS steps below)
...
4. If the images are private, `docker login` once on the server against GHCR and/or
   Docker Hub with a PAT.
5. Add the deploy user's public key to that account's `~/.ssh/authorized_keys`.
6. **GitHub Actions**: add repo secrets `DEPLOY_HOST`, `DEPLOY_USER`, `DEPLOY_SSH_KEY`
   (the matching private key). **Jenkins**: add an "SSH Username with private key"
   credential named `deploy-ssh-key`, and set `DEPLOY_HOST` as a Jenkins global/job
   environment variable.
7. Point `DOMAIN_APP`/`DOMAIN_ADMIN`/`DOMAIN_API`'s DNS A records at the server's IP.
8. **Bootstrap the first TLS certificates** (one-time, manual — nginx validates its
   ...
   no port conflict):
   ```bash
   cd /opt/grammarcetamol   # docker-compose.yml + docker/ already copied here (step 3/deploys)
...
9. `docker compose up -d` (with `COMPOSE_PROFILES=proxy` in `.env`, this now also starts
   nginx — which finds the certs step 8 just issued and comes up serving real HTTPS).
10. **Schedule renewals** — certs expire in 90 days; `docker/scripts/certbot-renew.sh` runs
...
untouched (deploys only ever refresh `docker-compose.yml` + `docker/`, never touch the
`certbot-certs` volume). To redeploy an older version by hand:
```bash
ssh <user>@<host>
...
both GHCR and Docker Hub, tagged `:latest` and `:<commit-sha>` — this needs
`DOCKERHUB_USERNAME`/`DOCKERHUB_TOKEN` added as repo secrets first; GHCR auth uses the built-in
`GITHUB_TOKEN`. A third phase then deploys to a cloud server over SSH — see
[Deploying to a cloud server](#deploying-to-a-cloud-server) above for the one-time server setup
and required `DEPLOY_HOST`/`DEPLOY_USER`/`DEPLOY_SSH_KEY` secrets. `Jenkinsfile` (repo root)
mirrors all three phases for a self-hosted Jenkins instance — point a Jenkins job at this repo
and configure the `dockerhub-creds`/`ghcr-creds`/`deploy-ssh-key` credentials it references.
