...

{
  ...
  if ! docker compose -p grammarcetamol ps --format json 2>/dev/null | grep -q .; then
    echo "Stack not running (no containers found) — skipping."
    exit 0
  fi

  STALLED=$(docker compose -p grammarcetamol ps -a --format '{{.Name}} {{.State}}' 2>/dev/null | grep -vE ' (running)$' || true)
  if [ -z "$STALLED" ]; then
