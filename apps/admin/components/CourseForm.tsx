    ...
    return (
        <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-6">
            <section className="bg-surface rounded-lg border border-border p-6 flex flex-col gap-4">
                ...
                <Input label="Title" value={values.title} onChange={(e) => updateValue('title', e.target.value)}
                       error={errors.title}/>
                <Input label="Subtitle" value={values.subtitle}
                    ...
                    {values.learningObjectives.length > 0 && (
                        <div className="flex flex-wrap gap-2 mt-2">
                            <Mapping array={values.learningObjectives} keyExtractor={(_, i) => i}>
                                {(objective, i) => (
                                    <span
                                        className="inline-flex items-center gap-1.5 bg-background border border-border rounded-full px-3 py-1 text-xs text-[#0F172A]">
                    {objective}
                                        <button type="button" onClick={() => removeObjective(i)}
                                                className="text-[#94A3B8] hover:text-error"
                ...
                <Input label="Target Audience" value={values.targetAudience}
                       onChange={(e) => updateValue('targetAudience', e.target.value)}/>
                <Input label="Prerequisites" value={values.prerequisites}
                       onChange={(e) => updateValue('prerequisites', e.target.value)}/>

                <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-1">
                        <label className="text-sm font-medium text-[#0F172A]">Category</label>
                        <select value={values.categoryId} onChange={(e) => updateValue('categoryId', e.target.value)}
                                className={selectClass}>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <Input label="Language" value={values.language}
                ...
                <Input label="Cover Image URL" value={values.coverImageUrl}
                       onChange={(e) => updateValue('coverImageUrl', e.target.value)}/>
                <Input label="Promo Video URL" value={values.promoVideoUrl}
                       onChange={(e) => updateValue('promoVideoUrl', e.target.value)}/>
                ...
                />
                <Input label="Instructor Bio" value={values.instructorBio}
                       onChange={(e) => updateValue('instructorBio', e.target.value)}/>
                <Input label="Instructor Avatar URL" value={values.instructorAvatarUrl}
                       onChange={(e) => updateValue('instructorAvatarUrl', e.target.value)}/>
            </section>
            ...
            <section className="bg-surface rounded-lg border border-border p-6 flex flex-col gap-4">
                ...
                <Input label="Currency" value={values.currency}
